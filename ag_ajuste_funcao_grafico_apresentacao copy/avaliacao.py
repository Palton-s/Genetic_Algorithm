...
def avaliacao(X, limites):
    ...
    y = [potential(0.05, 40, x[i]) for i in range(len(x))]

    for individuo in X: 

        """quadratic_sum = 0
        for i in range(len(x)):
            potential_ryd = potential(individuo[0], individuo[1], x[i])
            quadratic_sum += (potential_ryd - y[i])**2

        avaliacao = quadratic_sum/len(x)"""
        r2 = (individuo[0]-0.3)**2 + (individuo[1]-0.3)**2
        r1 = (individuo[0])**2 + (individuo[1])**2
        z = 0
...
"""x = [i/4 for i in range(-50, 50)]
y = [potential(1, 40, x[i]) for i in range(len(x))]
