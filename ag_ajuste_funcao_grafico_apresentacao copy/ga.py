...
class GA(Dados):
    ...
    def __init__(self, n_geracoes=Dados.n_geracoes, limites_dif=[], mudar_limites=False, N = 0) -> None:
        ...
        for i in range(self.n_geracoes):
            ...
            if mudar_limites and i % self.mudanca_limites == 0 and i != 0:
                ...
                self.values_temp = aux.converte_populacao(X, self.limites)
                for k in range(self.NV):
                    if limites_moveis[k]:
                        ...
                        retangulo.append([smallest_value[k], biggest_value[k] - smallest_value[k]])
                self.plotScarterAndSquare(retangulo, convergencia_count)
                convergencia_count += 1
    ...
    def plotScarterAndSquare(self, retangulo, convergencia_count):
        x_retangulo = retangulo[0][0]
        y_retangulo = retangulo[1][0]
        w_relangulo = retangulo[0][1]
        h_retangulo = retangulo[1][1]
        bests = self.values_temp

        #plot scarter
        x_s = [bests[i][0] for i in range(len(bests))]
        y_s = [bests[i][1] for i in range(len(bests))]
        plt.scatter(x_s, y_s, color="blue", alpha=0.2)
        #draw a rectangle
        rect = patches.Rectangle((x_retangulo, y_retangulo), w_relangulo, h_retangulo, linewidth=1, edgecolor='r', facecolor='none')

        # Add the patch to the Axes
        plt.gca().add_patch(rect)
        #title
        plt.title("Convergência " + str(convergencia_count))
        plt.show()
    ...
    def plot_result(self):
        ...
        y = [self.potential(0.05, 40, x[i]) for i in range(len(x))]
        ...
        for j in range(len(bests)):
            new_y = []
            new_x = [i/8 for i in range(-100, 100)]
            for i in range(len(new_x)):
                new_y.append(self.potential(bests[j][0], bests[j][1], new_x[i]))
            plt.plot(new_x, new_y, label='Melhor indivíduo', color='red', linewidth=0.5, alpha=0.5)
        ...
        #plot scarter
        x_s = [self.values[i][0] for i in range(len(self.values))]
        y_s = [self.values[i][1] for i in range(len(self.values))]
        plt.scatter(x_s, y_s, color="blue", alpha=0.2)
        plt.show()
        ...
        plt.title("Ajuste de todos indivíduos")
        for j in range(len(bests)):
            new_y = []
            new_x = [i/8 for i in range(-100, 100)]
            for i in range(len(new_x)):
                new_y.append(self.potential(bests[j][0], bests[j][1], new_x[i]))
            plt.plot(new_x, new_y, label='Melhor indivíduo', color='red', linewidth=0.5, alpha=0.2)
        plt.plot(x, y, label='Dados da curva')
        """for i in range(25):
            A = self.values[-i][0]
            B = self.values[-i][1]
        ...
        bests = self.values[450:480]

        plt.title("Ajuste dos indivíduos em mínimos locais")
        for j in range(len(bests)):
            new_y = []
            new_x = [i/8 for i in range(-100, 100)]
            for i in range(len(new_x)):
    ...
    def potential(self,A, B, x):
        result = A*x*x*np.sin((B/20)*x)*np.exp((-x**2)/B)
        return result
