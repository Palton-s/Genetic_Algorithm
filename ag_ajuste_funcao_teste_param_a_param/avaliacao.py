...
def avaliacao(X, limites, ultil):
    ...
    for individuo in X:
        ...
        for i in range(1, len(individuo)-1):
            a.append(individuo[i])
        ultil_pounts = int((ultil/100)*len(R))
        for i in range(ultil_pounts):
            potential_ryd = potential_2(D, a, R[i], R_ef)
            quadratic_sum += ((potential_ryd - HF[i])**2)*(0.99**(i))
            #print(quadratic_sum)
...
y_other = [potential_2(2.9342610000000002, [0.4457, 0.0310, 0.0438279,0.001,0.02244,-0.242],x[i],  1.4632) for i in range(len(x))]
