...
import mutacao
import populacao
import avaliacao_original as av
...
class Dados():    
    ...
    mudanca_limites = 20000
    NV = 2  # número de variáveis
    N = 100000 # numero de individuos
    n_bits = 18 # número de bits por variável em cada indivíduo
    percent_elite = 0.00025 # percentual dos melhores indivíduos que serão conservados para a próxima geração
...
class GA(Dados):
    ...
    def __init__(self, n_geracoes = Dados.n_geracoes, limites_dif = [], mudar_limites = False, N = [], NV=0 ) -> None:
        init_time_ex = time.time()
        ...
        for i in range(self.NV):
            self.limites.append([-100000, 100000])
        #super().__init__()
        # inicia a população
        ...
        for i in range(self.n_geracoes):
            ...
            #print("----------------------------------")
            #print("Tempo de execução: " + str(end_time_ex))
            #print(i, Y[-1])
        ...
        end_time_ex = time.time() - init_time_ex
        print(self.N, self.NV,end_time_ex)


for n in range(1,10):
    for nv in range(1,10):
        ga = GA(1, N=10**n, NV=nv)
