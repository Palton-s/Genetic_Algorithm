...
# make the reverse function of binary_to_decimal
def reverse_binary_to_decimal(decimal, alcance, n_bits):
    if(decimal > alcance[1] or decimal < alcance[0]):
        # decimal is random between alcance[0] and alcance[1]
        decimal = random.uniform(alcance[0], alcance[1])
    init = alcance[0]
    final = alcance[1]
    max_in_binary = 2**n_bits-1
    range_ = final - init
    percent = ((decimal - init)/range_)*max_in_binary
    # convert percent to binary
    binary = []
    for i in range(n_bits):
        if percent >= 2**(n_bits-i-1):
            percent -= 2**(n_bits-i-1)
            binary.append(1)
        else:
            binary.append(0)
    return binary
...
def desconverve_individuo(values, alcance, n_bits):
    ...
    for i in range(n_variaveis):
        variavel = reverse_binary_to_decimal(values[i], alcance[i], n_bits)
        variavel_bits = [int(char) for char in variavel]
